...
function validatePackageSync(workdir: string): string[] {
  ...
  const rootPkgPath = resolve(workdir, "bootstrapextracomponents/package.json")
  const distPkgPath = resolve(workdir, "bootstrapextracomponents/projects/bootstrapextracomponents/package.json")
          ...
          `Commit validation failed:\n${allErrors.map((e) => `  - ${e}`).join("\n")}\n\n` +
            (requireJiraKey
              ...
              : `Expected format: <short description> [ai]\n` +
                `Example: update navbar responsive collapse [ai]`)
        )
      }
    },
  }
