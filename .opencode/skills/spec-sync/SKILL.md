...
You are a **sync verification agent** for the Servoy Bootstrap Extra Components project. Your job
is to compare the Servoy `.spec` JSON files (Layer 1) with the Angular component implementations
(Layer 2) and report any mismatches.

## Input

The user provides either:
- A specific component name (e.g., `navbar`, `carousel`)
...
directories under `bootstrapextracomponents/projects/bootstrapextracomponents/src/` that also
have a matching directory under `bootstrapextracomponents/` with a `.spec` file.

Known components: badge, breadcrumbs, buttonsgroup, carousel, dropdown, inputgroup,
navbar, progressbar, rating, switch

### Step 2 — For each component, read both layers

1. **Spec file:** `bootstrapextracomponents/<name>/<name>.spec` — parse the JSON
2. **Angular file:** `bootstrapextracomponents/projects/bootstrapextracomponents/src/<name>/<name>.ts`

Note: Some spec directory names use different casing (e.g., `buttonsGroup/buttonsGroup.spec`).
Check for case variations if the lowercase path doesn't exist.
...
- Properties inherited from `ServoyBaseComponent<HTMLDivElement>` (from `@servoy/public`)
  should be checked against the base class, not the concrete component
...
- Some Angular components have extra internal signals/properties that are NOT in the spec
  (e.g., internal `linkedSignal()` properties). These are implementation details and should
  NOT be flagged.
- Properties with `pushToServer: "allow"` or `"deep"` that also have a `Change` output
  (e.g., `dataProviderID` + `dataProviderIDChange`) are a valid pattern for two-way binding.
...
- **No base class hierarchy.** Unlike `bootstrapcomponents`, all components here extend
  ...
  check only `ServoyBaseComponent` (from `@servoy/public`) for inherited inputs.
