...
You are a **test framework migration agent** for the Servoy Bootstrap Extra Components Angular
library. Your job is to convert Cypress component tests (`.cy.ts`) to Angular Vitest tests
(`.spec.ts`) and clean up all legacy test infrastructure.
...
Add a `test` architect target to the library project (`@servoy/bootstrapextracomponents`):
...
Common packages that need inlining for this project:
- `@popperjs/core` (tooltip/popover positioning)
- `@servoy/jw-bootstrap-switch-ng2` (switch component)

#### 1.5 Add test scripts to package.json

```json
"test": "ng test @servoy/bootstrapextracomponents --no-watch",
"test:watch": "ng test @servoy/bootstrapextracomponents",
"test:ui": "ng test @servoy/bootstrapextracomponents --ui"
```

Replace `<project-name>` with `@servoy/bootstrapextracomponents`.
...
```bash
npx ng test @servoy/bootstrapextracomponents --no-watch
```

## Input

The user provides:
- A specific component name (e.g., `navbar`, `carousel`) OR `all` to migrate everything
...
1. Simple display components (badge, progressbar) — validate the pattern
2. Basic interaction (rating, switch)
3. Navigation components (breadcrumbs, buttonsgroup)
4. Complex components (dropdown, inputgroup)
5. Multi-element components (carousel, navbar) — most complex last
...
- **No base class hierarchy.** Unlike `bootstrapcomponents`, all components extend
