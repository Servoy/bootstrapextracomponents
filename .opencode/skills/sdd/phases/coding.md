...
- **No base class hierarchy** — all components extend `ServoyBaseComponent<HTMLDivElement>`
  directly from `@servoy/public` (unlike bootstrapcomponents which has intermediate classes)

## Input

You receive a path to a spec file (e.g. `docs/SVY-21080-navbar-collapse.spec.md`).
...
- Extend `ServoyBaseComponent<HTMLDivElement>` directly from `@servoy/public`
- Use `ChangeDetectionStrategy.OnPush`
- Selector prefix: `bootstrapextracomponents-` (kebab-case)
- Component is `standalone: false`, declared in `ServoyBootstrapExtraComponentsModule`
- Custom types use `BaseCustomObject` from `@servoy/public`
...
1. Add to `ServoyBootstrapExtraComponentsModule` declarations in `servoybootstrapextra.module.ts`
2. Add to exports in `public-api.ts`

### 6. Post-edit verification

After all changes are done:
1. Run `npx ng build @servoy/bootstrapextracomponents` from `bootstrapextracomponents/` to
...
```
- projects/bootstrapextracomponents/src/navbar/navbar.ts (modified)
- navbar/navbar.spec (modified)
- projects/bootstrapextracomponents/src/public-api.ts (modified)
- ...
```
