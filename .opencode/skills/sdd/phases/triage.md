...
Key areas to investigate for this Angular component library:
- The Servoy component spec file (`bootstrapextracomponents/<name>/<name>.spec`) — defines
  the component contract (model properties, handlers, API methods, types)
- The Angular implementation (`bootstrapextracomponents/projects/bootstrapextracomponents/src/<name>/`)
- The `@servoy/public` base classes and utilities
- Note: All components extend `ServoyBaseComponent<HTMLDivElement>` directly — there is NO
  intermediate base class hierarchy in this project
- The component's template (`.html`) for rendering logic
...
```powershell
cd "C:\Users\jcomp\git\bootstrapextracomponents" ; git blame -L <start>,<end> "<file-path>"
...
- **Is the problem even in Servoy component code?** Or is it user-side (misconfiguration,
  ...
  (`@servoy/public`, `@ng-bootstrap`, `@popperjs/core`, etc.)?
